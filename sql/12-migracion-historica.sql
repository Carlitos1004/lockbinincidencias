...
-- Clave natural para evitar duplicados si la migración se corre más de una
-- vez por error: no puede haber dos filas de garantía para el mismo
-- OT + Módulo + Dispositivo dañado. (Con manejo de excepción para que sea
-- seguro correr esto de nuevo aunque ya exista.)
do $$
begin
  alter table garantias add constraint garantias_clave_natural
    unique (id_ot, m_control, dispositivo_danado);
exception
  when duplicate_object then null;
end $$;

-- Lo mismo para Componentes_Retirados, para que la migración se pueda
-- reintentar sin duplicar lo que ya se importó.
do $$
begin
  alter table componentes_retirados add constraint componentes_retirados_clave_natural
    unique (id_registro, tipo_componente, serial_retirado);
exception
  when duplicate_object then null;
end $$;
